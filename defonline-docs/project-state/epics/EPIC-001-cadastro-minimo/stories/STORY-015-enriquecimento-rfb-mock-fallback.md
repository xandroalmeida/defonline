...
- [x] **CA-4:** Cada consulta emite **métrica em `business_metrics`** (tabela existente do EPIC-000) com `tipo: 'rfb_consulta'` e `meta->>'status' ∈ {'sucesso' | 'cnpj_inexistente' | 'timeout' | 'erro_5xx' | 'erro_rede'}`. Latência (ms) registrada. **Sem CNPJ em log** (PII conforme `security-discipline.md`); apenas o hash SHA-256 do CNPJ no `audit_logs` se necessário para correlação.
