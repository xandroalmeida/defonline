...
5. **Rascunho persistido:** ao mudar de bloco (next/back) ou após X segundos de inatividade, payload parcial é salvo em `diagnosticos` com `status = 'rascunho'`. Roberto que volta dias depois vê os dados que digitou no último bloco.
...
- [ ] **CA-6 (Rascunho persistido):** ao apertar `Próximo`, payload parcial salva em `diagnosticos` com `status='rascunho'`. Voltar à tela após 1h mostra os dados preenchidos.
- [ ] **CA-7 (Expiração do rascunho — §6.4):** rascunho expira em 90 dias (default da spec). Listagem futura de diagnósticos só mostra rascunhos vivos. Decisão pode mudar via IDR.
...
- [ ] **CA-9 (Idempotência):** se Roberto submete o mesmo quiz duas vezes em sequência (refresh), o sistema reusa o diagnóstico existente em vez de criar duplicata. Chave: hash do `quiz_payload` + `empresa_id` + `motor_version`.
