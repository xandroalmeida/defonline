...
- [ ] **CA-4 (gate validação externa):** ~~STORY-036 está `approved` ou `approved_with_pending` com follow-ups não-críticos documentados.~~ **Decisão PO 2026-05-26:** validação externa formal foi adiada para após o fechamento técnico do épico (ver STORY-036). Este CA agora exige (a) STORY-036 registrada como `pending_external_validation` no `index.json`, (b) condição explícita no handoff (CA-6) de que **beta fechado não pode rodar até o parecer externo voltar `approved`/`approved_with_pending`**.
- [ ] **CA-5 (p95 ≤ 3s comprovado):** medição final em 50 navegações em homol, p95 ≤ 3s. Gráfico/tabela anexada.
- [ ] **CA-6 (handoff):** `handoff/README.md` cobre os 9 pontos do item 3 acima. ~~Dry-run com 1 representante de implantação na Semana 4.~~ **Decisão PO 2026-05-26:** sem dry-run interno neste épico — o handoff é entregue como leitura direta. Comercial/implantação assume o pacote quando o beta começar (que por sua vez depende da validação externa — ver CA-4). Risco de "lacunas só visíveis no uso real" registrado explicitamente no Bloco H-2 do `validation/checklist.md` e na seção de riscos do `handoff/README.md`.
