...
- 2026-05-24 — **aditivo (STORY-025, não muda a decisão):** o `infra/docker/Dockerfile` passa a ter dois targets finais. `runtime` (target deployado em homol/prod) é a imagem única descrita por este ADR — web/worker/scheduler continuam compartilhando exatamente a mesma imagem, só mudando o `command`. `dev` é um superset estritamente local: `FROM runtime` + `chromium` + `chromedriver`, usado pelo `docker-compose.yml` porque o pre-push roda Dusk via `docker compose exec -T web php artisan dusk` dentro do container `web`. O CI smoke (`release-homolog.yml`) usa chromium do runner do GitHub Actions, não do container — então o `runtime` em homol/prod sai ~750MB de layer mais leve sem perder nenhuma capacidade real. "Imagem única" do ADR continua valendo: é o `target runtime`. O `dev` é convenção de build local, não um segundo perfil de produção.
